...
func (s *Site) adminRewards(w http.ResponseWriter, r *http.Request) {
	...
	if err == nil {
		...
		for rows.Next() {
			var rwd rewardView
			_ = rows.Scan(&rwd.ID, &rwd.Title, &rwd.Description, &rwd.PointsCost, &rwd.Category, &rwd.Active, &rwd.MaxRedemptionsPerUser)
...
func (s *Site) adminRewardCreate(w http.ResponseWriter, r *http.Request) {
	...
	maxPerUser, _ := strconv.Atoi(strings.TrimSpace(r.FormValue("max_redemptions_per_user")))
	...
	if cost < 0 {
		http.Redirect(w, r, "/admin/rewards?error=Стоимость%20не%20может%20быть%20отрицательной", http.StatusSeeOther)
		return
	}
	if maxPerUser < 0 {
		http.Redirect(w, r, "/admin/rewards?error=Лимит%20получений%20не%20может%20быть%20отрицательным", http.StatusSeeOther)
		return
	}

	_, err := s.DB.Exec(
		`insert into rewards (title, description, points_cost, category, active, max_redemptions_per_user)
...
func (s *Site) adminPlanPauseSick(w http.ResponseWriter, r *http.Request) {
	...
	result, _ := s.DB.Exec(
     ...
     set scheduled_date = scheduled_date + $1::int
     where plan_id = $2
       and status = 'pending'
	...
	changeReason := fmt.Sprintf(
		"Больничный %s — %s: перенос по больничному на %d календарных дн. (перенесено: %d)",
		fromDate.Format("02.01.2006"),
		toDate.Format("02.01.2006"),
		daysCount,
		shifted,
	...
	adminPlanRedirect(w, r, userID, "success", fmt.Sprintf("План приостановлен по больничному: перенос на %d календарных дн.", daysCount))
}
...
func (s *Site) adminPlanWorkoutReschedule(w http.ResponseWriter, r *http.Request) {
	...
	scheduledTimeRaw := strings.TrimSpace(r.FormValue("scheduled_time"))
	if scheduledDateRaw == "" {
	...
	if scheduledTimeRaw != "" {
		if _, err := time.Parse("15:04", scheduledTimeRaw); err != nil {
			adminPlanRedirect(w, r, userID, "error", "Некорректное время тренировки")
			return
		}
	}
	...
	result, err := s.DB.Exec(
     ...
     set scheduled_date = $1,
         scheduled_time = case when trim($2) = '' then null else $2::time end
	...
	changedTo := scheduledDate.Format("02.01.2006")
	if scheduledTimeRaw != "" {
		changedTo += " " + scheduledTimeRaw
	}
	s.logPlanChange(
		userID,
		planID,
		"reschedule",
		"Администратор изменил дату тренировки на "+changedTo,
		before,
		after,
	)
