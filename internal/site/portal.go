...
func (s *Site) rewardsPage(w http.ResponseWriter, r *http.Request) {
	...
	data["Points"] = points
	data["PointsUsageHint"] = "При обмене поощрения баллы списываются с вашего текущего баланса."

	var rows, err = s.DB.Query(
		`select id, title, description, points_cost, coalesce(category, ''),
            coalesce(max_redemptions_per_user, 0)
     from rewards
     where active = true
     order by
       case when points_cost <= $1 then 0 else 1 end,
       points_cost,
       title`,
		points,
	)
	...
	if err == nil {
		...
		for rows.Next() {
			var rwd rewardView
			_ = rows.Scan(&rwd.ID, &rwd.Title, &rwd.Description, &rwd.PointsCost, &rwd.Category, &rwd.MaxRedemptionsPerUser)
			rewards = append(rewards, rwd)
		}
	}

	statsByReward := map[string]rewardView{}
	rows, err = s.DB.Query(
		`select reward_id,
	...
	if err == nil {
		...
		for rows.Next() {
			...
			var hasPending bool
			_ = rows.Scan(&rewardID, &approvedCount, &hasPending)
			statsByReward[rewardID] = rewardView{
	...
	latestStatusByReward := map[string]string{}
	rows, err = s.DB.Query(
		`select distinct on (reward_id) reward_id, status
     from reward_redemptions
     where user_id = $1
     order by reward_id, coalesce(handled_at, redeemed_at) desc, redeemed_at desc`,
		user.ID,
	)
	if err == nil {
		defer rows.Close()
		for rows.Next() {
			...
			latestStatusByReward[rewardID] = status
		}
	}

	for idx := range rewards {
		stats := statsByReward[rewards[idx].ID]
		rewards[idx].ApprovedCount = stats.ApprovedCount
		rewards[idx].HasPending = stats.HasPending
		rewards[idx].LatestStatus = latestStatusByReward[rewards[idx].ID]
		rewards[idx].LimitReached = rewards[idx].MaxRedemptionsPerUser > 0 && rewards[idx].ApprovedCount >= rewards[idx].MaxRedemptionsPerUser
	}
...
func (s *Site) rewardRedeem(w http.ResponseWriter, r *http.Request) {
	...
	if err := db.EnsureUserDefaults(s.DB, user.ID); err != nil {
		http.Redirect(w, r, "/rewards?error=Не%20удалось%20подготовить%20профиль", http.StatusSeeOther)
		return
	}

	tx, err := s.DB.Begin()
	if err != nil {
		http.Redirect(w, r, "/rewards?error=Не%20удалось%20создать%20заявку", http.StatusSeeOther)
		return
	}
	defer tx.Rollback()
	...
	var maxPerUser int
	err = tx.QueryRow(
		`select points_cost, coalesce(max_redemptions_per_user, 0)
     from rewards
     where id = $1 and active = true`,
		rewardID,
	).Scan(&cost, &maxPerUser)
	if err != nil {
	...
	if err != nil {
		http.Redirect(w, r, "/rewards?error=Не%20удалось%20проверить%20баланс", http.StatusSeeOther)
		return
	}
	...
	_ = tx.QueryRow(
		`select count(*)
     from reward_redemptions
     where user_id = $1 and reward_id = $2 and status = 'pending'`,
		user.ID,
		rewardID,
	).Scan(&pendingCount)
	if pendingCount > 0 {
		http.Redirect(w, r, "/rewards?error=Заявка%20по%20этому%20поощрению%20уже%20на%20рассмотрении", http.StatusSeeOther)
		return
	}
	...
	_ = tx.QueryRow(
		`select count(*)
     from reward_redemptions
     where user_id = $1 and reward_id = $2 and status = 'approved'`,
		user.ID,
		rewardID,
	).Scan(&approvedCount)
	if maxPerUser > 0 && approvedCount >= maxPerUser {
		http.Redirect(w, r, "/rewards?error="+url.QueryEscape(fmt.Sprintf("Лимит получения этого поощрения исчерпан: %d", maxPerUser)), http.StatusSeeOther)
		return
	}
	...
	if _, err = tx.Exec(
		...
		http.Redirect(w, r, "/rewards?error=Не%20удалось%20списать%20баллы", http.StatusSeeOther)
		return
	}
	if err = tx.Commit(); err != nil {
		http.Redirect(w, r, "/rewards?error=Не%20удалось%20сохранить%20заявку", http.StatusSeeOther)
		return
	}
...
func (s *Site) managerDashboard(w http.ResponseWriter, r *http.Request) {
	...
	rewardReceivedMap := map[string]int{}
	rewardsQuery := `select rr.user_id, count(*)
                   from reward_redemptions rr
                   join users u on u.id = rr.user_id
                   where rr.status = 'approved'`
	args = []any{}
	if user.Role == "manager" && department != "" {
		rewardsQuery += " and u.department = $1"
		args = append(args, department)
	}
	rewardsQuery += " group by rr.user_id"

	rows, err = s.DB.Query(rewardsQuery, args...)
	if err == nil {
		defer rows.Close()
		for rows.Next() {
			var userID string
			var count int
			_ = rows.Scan(&userID, &count)
			rewardReceivedMap[userID] = count
		}
	}
	...
	for i := range employees {
		...
		employees[i].RewardsReceived = rewardReceivedMap[employees[i].ID]
		totalRewardsReceived += employees[i].RewardsReceived
	}
	data["Employees"] = employees
	data["DepartmentRewardsReceived"] = totalRewardsReceived
...
func (s *Site) managerEmployee(w http.ResponseWriter, r *http.Request) {
	...
	rewardHistory := []managerRewardHistoryView{}
	rows, err = s.DB.Query(
		`select coalesce(u.name, ''),
            coalesce(r.title, ''),
            coalesce(r.points_cost, 0),
            coalesce(rr.handled_at, rr.redeemed_at),
	...
	if err == nil {
		defer rows.Close()
		for rows.Next() {
			var v managerRewardHistoryView
			var receivedAt time.Time
			_ = rows.Scan(&v.EmployeeName, &v.RewardTitle, &v.PointsCost, &receivedAt, &v.ApprovedBy)
			v.ReceivedAt = receivedAt.Format("02.01.2006 15:04")
			rewardHistory = append(rewardHistory, v)
		}
	}
...
func (s *Site) managerAward(w http.ResponseWriter, r *http.Request) {
	...
	if reason == "" {
		reason = "Начисление руководителем"
	}
	if _, err := tx.Exec(
		`insert into user_point_events (user_id, source, points, reason, created_by)
     values ($1, 'manager_award', $2, nullif($3, ''), $4)`,
		employeeID,
		points,
		reason,
		user.ID,
...
func (s *Site) managerRedemptionReject(w http.ResponseWriter, r *http.Request) {
	...
	_, _ = s.DB.Exec(
		`insert into user_point_events (user_id, source, points, reason, created_by)
     values ($1, 'reward_rejection_refund', $2, 'Возврат баллов за отклонённую заявку на поощрение', $3)`,
		userID,
		cost,
		user.ID,
	)
...
func (s *Site) adminDashboard(w http.ResponseWriter, r *http.Request) {
	...
	rows, err = s.DB.Query(
		`select u.id, u.name, u.employee_id, coalesce(u.corporate_email, ''), u.role, coalesce(u.department, ''), coalesce(u.position, ''),
	            ...
	            coalesce(extract(year from age(current_date, up.birth_date))::int, 0),
	...
	if err == nil {
		...
		for rows.Next() {
			...
			_ = rows.Scan(&u.ID, &u.Name, &u.EmployeeID, &u.CorporateEmail, &u.Role, &u.Department, &u.Position, &u.BirthDate, &u.Age, &u.DoctorApproval)
...
func (s *Site) adminUserCreate(w http.ResponseWriter, r *http.Request) {
	...
	corporateEmail, emailErr := normalizeCorporateEmail(r.FormValue("corporate_email"))
	if emailErr != nil {
		http.Redirect(w, r, "/admin?error=Укажите%20корректную%20корпоративную%20почту", http.StatusSeeOther)
		return
	}
	...
	var userID string
	err = s.DB.QueryRow(
		`insert into users (name, employee_id, corporate_email, password_hash, role, department, position, password_temp)
     values ($1, $2, nullif($3, ''), $4, $5, nullif($6, ''), nullif($7, ''), true)
     returning id`,
		name,
		employeeID,
		corporateEmail,
	...
	if err != nil {
		http.Redirect(w, r, "/admin?error=Табельный%20номер%20уже%20занят", http.StatusSeeOther)
		return
	}
